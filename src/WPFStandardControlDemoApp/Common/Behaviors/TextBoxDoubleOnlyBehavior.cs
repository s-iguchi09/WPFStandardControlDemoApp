...
{
    /// <summary>
    /// TextBox に対して数値（double 型）以外の入力を禁止するビヘイビアです。
    /// 不正な文字入力を自動的に抑制し、数値入力専用の TextBox を実現します。
    /// </summary>
    public static class TextBoxDoubleOnlyBehavior
    {
        public static readonly DependencyProperty EnableProperty =
            DependencyProperty.RegisterAttached(
                "Enable",
                typeof(bool),
                typeof(TextBoxDoubleOnlyBehavior),
        ...
        private static void OnEnableChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                throw new InvalidOperationException("TextBoxDoubleOnlyBehavior is for TextBox only.");
            }
        ...
        private static void OnPaste(object sender, DataObjectPastingEventArgs e)
        {
            ...
            if (e.DataObject.GetDataPresent(DataFormats.Text))
            {
                ...
                if (!IsValidDouble(paste, allowNegative))
                {
                    e.CancelCommand();
                }
            }
        ...
        private static bool IsValidDouble(string text, bool allowNegative)
        {
